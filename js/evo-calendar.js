...
(function (factory) {
	...
	// v1.0.0 - Format date
	EvoCalendar.prototype.formatDate = function (
		date,
		format,
		language
	) {
	...
	EvoCalendar.prototype.isBetweenDates = function (
		active_date,
		dates
	) {
		var sd, ed;
	...
	EvoCalendar.prototype.removeEventList = function (event_data) {
		...
		eventListEl
			.find('[data-event-index="' + event_data + '"]')
			.remove();
		if (eventListEl.find("[data-event-index]").length === 0) {
			eventListEl.empty();
			if (_.$active.date === _.$current.date) {
	...
	EvoCalendar.prototype.buildCalendar = function () {
		...
		title = _.formatDate(
			new Date(
				_.$label.months[_.$active.month] + " 1 " + _.$active.year
			),
	...
	EvoCalendar.prototype.addEventIndicator = function (event) {
		...
		function appendDot(date) {
			...
			if (
				thisDate.find(
					"span.event-indicator > .type-bullet > .type-" + type
				).length === 0
			) {
	...
	EvoCalendar.prototype.selectEvent = function (event) {
		...
		if (modified_event.date instanceof Array) {
			modified_event.dates_range = _.getBetweenDates(
				modified_event.date
			);
		}
